...
public class UserRsocketController {
        ...
        return jwtToken
                .flatMap(
                        jwt -> {
                            ...
                            JSONArray scope = ((JSONArray) jwt.getClaims().get("scope"));
                            String[] rules = new String[scope.size()];
                            for (int i = 0; i < scope.size(); i++) {
                                rules[i] = scope.get(i).toString();
                            }
    ...
    public Flux<UserDTO> users(
        ...
        return jwtToken.flatMap(jwt -> {
            ...
            Object[] roles = ((JSONArray) claims.get("scope")).toArray();
