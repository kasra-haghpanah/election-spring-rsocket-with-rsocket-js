...
public class UserController {
    ...
    public Mono<JwtResponse> cookie(
        ...
        return jwtToken.flatMap(jwt -> {
            Object[] roles = ((JSONArray) jwt.getClaims().get("scope")).toArray();
    ...
    @RequestMapping(value = "/signin", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Mono<JwtResponse> signin(
        ...
        return exchange.getFormData().flatMap(form -> {
            String username2 = form.get("username").get(0);
            while (username2.length() > 1 && username2.indexOf("0") == 0) {
                username2 = username2.substring(1, username2.length());
            }
            String password2 = form.get("password").get(0);
            return userService.findByUsername(username2)
                    .flatMap(userDetails -> {
                        if (userDetails != null && passwordEncoder.matches(password2, userDetails.getPassword())) {
                            int level = userDetails.getRoles().contains("ADMIN") ? 0 : 1;
                            JwtResponse response = new JwtResponse(JwtConfig.encoder(userDetails), userDetails.getUsername(), level, serverDomainBaseUrl, userDetails.getMetadata());
...
//        }
        return jwtToken.flatMap(jwt -> {
            ...
            JSONArray roles = (JSONArray) claims.get("scope");
